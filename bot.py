...
# ── 🆕 سرو فایل عمومی برای «لینک پابلیک از سرور خودم» (file_server.py) ──
# مسیر /f/<token> با پشتیبانی Range سرو میشه → پخش و Seek در VLC کار می‌کنه.
# دامنه‌ی عمومی از متغیر محیطی PUBLIC_BASE_URL خونده میشه (تو Railway Variables
# همون دامنه‌ای که Generate Domain میده رو بذار).
try:
    from file_server import register_flask_routes as _fs_register
    _fs_register(flask_app)
except Exception as _fs_err:  # file_server نبود یا خطا — بات بدونش هم بالا میاد
    print(f"[FileServer] route registration failed: {_fs_err}")


def start_keep_alive():
    def _run():
        try:
            print(f"[KeepAlive] HTTP server (health + /f/<token>) listening on 0.0.0.0:{HEALTH_PORT}")
            print(f"[KeepAlive] ⚠️ PUBLIC_BASE_URL = {os.environ.get('PUBLIC_BASE_URL', '(not set)')} — پورت دامنه باید همون {HEALTH_PORT} باشه")
            flask_app.run(host="0.0.0.0", port=HEALTH_PORT, debug=False)
        except Exception as e:
...
async def ytdirect_video_callback(event):
    ...
    try:
        ...
        if not ok or not os.path.exists(res):
            # ── fallback خودکار به SnapWC وقتی مسیر مستقیم جواب نمی‌ده ──
            logger.warning(
                "[YtDirect] direct video failed (%s) — falling back to SnapWC", res
            )
            await safe_edit(
                status_msg,
                "⚠️ مسیر مستقیم برای این ویدیو جواب نداد.\n"
                "🎭 در حال تلاش از طریق SnapWC (مرورگری)...",
                buttons=None,
            )
            ytdirect_sessions.pop(session_id, None)
            try:
                await _run_snapwc_flow(event, sess["url"], status_msg)
            except Exception as snap_err:
                logger.error(f"[YtDirect] snapwc fallback error: {snap_err}", exc_info=True)
                await safe_edit(status_msg, f"❌ هر دو مسیر شکست خوردن: {str(res)[:100]}")
            return
...
async def ytdirect_audio_callback(event):
    ...
    try:
        ...
        if not ok or not os.path.exists(res):
            # ── fallback خودکار به SnapWC برای آدیو ──
            logger.warning(
                "[YtDirect] direct audio failed (%s) — falling back to SnapWC", res
            )
            await safe_edit(
                status_msg,
                "⚠️ مسیر مستقیم برای این آدیو جواب نداد.\n"
                "🎭 در حال تلاش از طریق SnapWC (مرورگری)...",
                buttons=None,
            )
            ytdirect_sessions.pop(session_id, None)
...
async def _flush_video_send_batch(
    ...
    if count == 1:
        ...
        # ☁️ آپلود ابری — زنجیره‌ی fallback جدید:
        # سرور خودم → پیکسل‌درین → Litterbox → Catbox → Uguu → Gofile
        # (هندلر fvlc_ خودش پیام وضعیت + نوار پیشرفت نشون میده)
        buttons.append([
            Button.inline("☁️ آپلود ابری (لینک مستقیم)", f"fvlc_{chat_id}_{files[0]['message_id']}"),
        ])
        # ☁️ Filebin — مثل قبل سر جاشه
        buttons.append([
    ...
    # دکمه آپلود به uplod.ir — مثل قبل سر جاشه
