"""
rapeinass_handler.py
────────────────────
استخراج و دانلود ویدیو از rapeinass.com

روش کار (بر اساس تحلیل واقعی سایت — تأیید شده با تست):

  سایت ساده — بدون پلاگین پلیر اختصاصی. ویدیو با HTML5 native <video> embed می‌شه.

  ساختار URL:
    /<category>/<video_id>-<slug>.html
    یا
    /<category>/<video_id>.html

  مثال:
    /incest-rape/842586139-Incest-rape-porn-videos.html

  HTML الگو:
    <video controls controlslist="nodownload" playsinline
           disablepictureinpicture class="nativeVideo"
           poster="https://media.rapeinass.com/8/4/842586139/content/10000.jpg"
           src="https://media.rapeinass.com/8/4/842586139/content/full.mp4?ahk=<token>">
    </video>

  CDN (media.rapeinass.com):
    URL pattern: https://media.rapeinass.com/<d1>/<d2>/<video_id>/content/full.mp4?ahk=<token>
      d1 = first digit of video_id
      d2 = second digit of video_id

    Token (ahk):
      pattern: <base64-like>kkk@kkki<timestamp>
      مثال: UsfSo1X94eFUNRPGY1xpPAkkk@kkki1785631851
      timestamp = expiry time (4 ساعت از زمان fetch صفحه)
      IP-locked: نیست (احتمالاً)
      Referer: الزامی!  بدون Referer یا با fake Referer → 403

    Accept-Ranges: bytes — پشتیبانی از Range requests
    Content-Type: video/mp4

  نکات:
    - سایت تک‌کیفیتیه
    - کوکی `from` set می‌شه ولی برای دانلود لازم نیست
    - توکن 4 ساعت معتبره — handler در صورت 403 خودش صفحه رو دوباره fetch می‌کنه

استراتژی دانلود:
  1. GET parent page برای metadata و video URL
  2. استخراج src از <video> tag
  3. multi-segment download با N workers (پیش‌فرض 16)
  4. fallback به single-connection
  5. fallback به yt-dlp روی page URL
  6. در 403: re-fetch parent page برای token تازه
...
logger = logging.getLogger("RapeInAssHandler")
...
# CDN host
_CDN_HOST = "media.rapeinass.com"

# توکن 4 ساعت معتبره — ما 3.5 ساعت در نظر می‌گیریم برای margin
_TOKEN_REFRESH_THRESHOLD = int(3.5 * 3600)  # seconds
...
def is_rapeinass_url(url: str) -> bool:
    """بررسی اینکه URL مربوط به rapeinass.com هست."""
    try:
        host = urlparse(url).hostname or ""
        return host in _ALLOWED_HOSTS or host.endswith(".rapeinass.com")
    except Exception:
        return False
...
def extract_video_id(url: str) -> Optional[str]:
    ...
    الگوها:
        /<category>/<video_id>-<slug>.html  →  video_id
        /<category>/<video_id>.html          →  video_id

    مثال: /incest-rape/842586139-Incest-rape-porn-videos.html → 842586139
    """
    try:
        parsed = urlparse(url)
        path = parsed.path or ""
        # الگوی /<category>/<digits>(-...).html
        m = re.search(r"/\d+-(\d+)[^/]*\.html", path)
        if not m:
            # الگوی /<category>/<digits>.html (بدون slug)
            m = re.search(r"/(\d+)\.html", path)
        if not m:
            # الگوی /<digits>-  در مسیر
...
def _token_seconds_remaining(video_url: str) -> Optional[int]:
    ...
    الگوی توکن: ahk=<base64>kkk@kkki<timestamp>
    timestamp انتهای توکنه.
    """
    m = re.search(r"ahk=[^&]+?(\d{10})(?:&|$)", video_url)
    if not m:
        return None
    try:
...
def _strip_site_suffix(title: str) -> str:
    """حذف پسوندهای سایت."""
    title = re.sub(r"\s*[-|@]\s*rapeinass(?:\.com)?\s*$", "", title, flags=re.IGNORECASE)
    return title.strip()


def _extract_title(html: str) -> str:
    """استخراج عنوان ویدیو."""
    # روش 1: <h1> (معمولاً عنوان ویدیو)
...
def _extract_duration(html: str) -> Optional[int]:
    ...
    # روش 2: data-length attribute (الگوی رایج در این سایت)
    # <span data-length="6:25">6:25</span>
    m = re.search(r'data-length=["\'](\d{1,2}:\d{2}(?::\d{2})?)["\']', html)
    if m:
        d = _parse_time_string(m.group(1))
        if d:
            return d

    # روش 3: og:video:duration
    m = re.search(
...
def _extract_video_source(html: str) -> Optional[dict]:
    """
    استخراج URL ویدیو از <video src="..."> tag.

    الگوی HTML:
        <video controls ... class="nativeVideo"
               poster="..."
               src="https://media.rapeinass.com/.../full.mp4?ahk=...">
        </video>

    Returns:
        dict با کلیدهای url, poster, method یا None
    """
    # روش 1: پیدا کردن <video> با src attribute
    # الگوی دقیق برای media.rapeinass.com
    video_match = re.search(
        r'<video\b[^>]*\bsrc=["\']([^"\']*media\.rapeinass\.com[^"\']+\.mp4[^"\']*)["\'][^>]*>',
        html, re.IGNORECASE,
    )
    if not video_match:
        # fallback: هر <video> با src که .mp4 داشته باشه
        video_match = re.search(
            r'<video\b[^>]*\bsrc=["\']([^"\']+\.mp4[^"\']*)["\'][^>]*>',
            html, re.IGNORECASE,
        )
    ...
    if not video_match:
        ...
        url_match = re.search(
            r'(https?://[^\s"\'<>)\]]*media\.rapeinass\.com[^\s"\'<>)\]]+\.mp4[^\s"\'<>)\]]*)',
            html, re.IGNORECASE,
        )
        if not url_match:
            # fallback نهایی: هر URL با ahk= token
            url_match = re.search(
                r'(https?://[^\s"\'<>)\]]+?\.mp4\?[^\s"\'<>)\]]*ahk=[^\s"\'<>)\]]+)',
                html, re.IGNORECASE,
            )
        if url_match:
            url = _clean_url(url_match.group(1))
            # استخراج poster از همون video tag
            poster = _extract_poster_near(html, url_match.start())
    ...
    # استخراج poster از همون video tag
    poster = _extract_poster_near(html, video_match.start())

    logger.info("Found video URL from <video src=>: %s", url[:100])
    return {
        "url": url,
        "poster": poster,
        "method": "video_src_tag",
    }


def _extract_poster_near(html: str, pos: int) -> str:
    """استخراج poster از video tag در نزدیکی position."""
    # پیدا کردن end of video tag
    end = html.find('>', pos)
    if end < 0:
        return ""
    video_tag = html[pos:end+1]
    m = re.search(r'\bposter=["\']([^"\']+)["\']', video_tag, re.IGNORECASE)
    if m:
        return m.group(1).strip()
    return ""
...
async def extract_rapeinass_qualities(
    url: str,
    progress_cb: Optional[ProgressCallback] = None,
) -> Tuple[List[dict], str, dict]:
    """
    استخراج کیفیت‌های ویدیو از rapeinass.com.
    ...
    Returns:
        ...
        title: عنوان ویدیو
        info: dict با کلیدهای thumbnail/page_url/video_id/duration
    """
    if not is_rapeinass_url(url):
    ...
    # اگه thumbnail از og:image پیدا نشد، از poster استفاده کن
    if not thumbnail and source_info.get("poster"):
        thumbnail = source_info["poster"]
    ...
    token_remaining = _token_seconds_remaining(source_info["url"])
    if token_remaining is not None:
        logger.info("Token expires in %d seconds", token_remaining)
        if token_remaining < 300:
            logger.warning("Token expires soon (%ds)", token_remaining)
    ...
    if progress_cb:
        ...
        msg = f"✅ **پیدا شد:** {title[:50]}{dur_str}\n🎞 کیفیت‌ها: {labels}"
        if token_remaining is not None:
            mins, secs = divmod(token_remaining, 60)
            hours = mins // 60
            mins = mins % 60
            msg += f"\n⏳ توکن تا {hours}:{mins:02d}:{secs:02d} معتبره"
...
💾 Size: {total_mb:.1f} MB\n🔥 {num_workers} parallel workers"
        ...
        logger.info(
            "[DL-RAPEINASS] Work-queue: %d chunks, %d workers, total=%d",
...
`[{bar}]`\n"
        ...
        try:
            ...
            logger.error("[DL-RAPEINASS] Work-queue error: %s", e, exc_info=True)
        ...
        if worker_failures or failed_chunks:
            logger.warning(
                "[DL-RAPEINASS] %d workers failed, %d chunks failed",
        ...
        logger.info(
            "[DL-RAPEINASS] Multi-segment DONE | size=%s | time=%.1fs | avg=%.1f MB/s",
            _format_size(file_size), elapsed, avg_speed,
        )
        return True, "", file_size

    except Exception as e:
        logger.error("[DL-RAPEINASS] Multi-segment error: %s", e, exc_info=True)
        _cleanup_file(filepath)
        return False, str(e)[:200], 0
...
async def _make_worker_task(worker_func, worker_id: int):
    ...
    try:
        ...
        logger.error("[DL-RAPEINASS] Worker %d crashed: %s", worker_id, e)
        return False
        ...
        while True:
            ...
            for attempt in range(max_retries):
                ...
                try:
                    ...
                    logger.warning(
                        "[DL-RAPEINASS] W%d c%d attempt %d failed: %s",
    ...
    if _check_curl_cffi():
        try:
            ...
            async with _CurlAsyncSession() as session:
                ...
                logger.info(
                    "[DL-RAPEINASS] Single (curl_cffi) DONE | size=%s | time=%.1fs",
                    _format_size(size), elapsed,
                )
                return True, "", size

        except Exception as e:
            logger.warning(f"[DL-RAPEINASS] Single (curl_cffi) error: {e}")
            _cleanup_file(filepath)
...
async def _download_with_ytdlp(url: str, filepath: str, progress_cb: ProgressCallback) -> Tuple[bool, str, int]:
    ...
    try:
        cmd = [
            ...
            "--add-header", f"Referer:https://rapeinass.com/",
            "-o", filepath,
        ]
        if has_curl_cffi:
        ...
        if actual_path != filepath:
            try:
                ...
                pass
        logger.info("[DL-RAPEINASS] yt-dlp DONE | size=%s", _format_size(size))
        return True, "", size
        ...
        logger.error("[DL-RAPEINASS] yt-dlp error: %s", e, exc_info=True)
        _cleanup_file(filepath)
        return False, str(e)[:200], 0
...
async def download_rapeinass_video(
    page_url: str,
    video_url: str,
    filepath: str,
    progress_cb: Optional[ProgressCallback] = None,
    ...
    """
    دانلود ویدیو از rapeinass.com.

    Args:
        page_url: URL صفحه ویدیو (برای Referer)
        video_url: URL مستقیم MP4 (از extract_rapeinass_qualities)
    ...
    نکته مهم:
        CDN media.rapeinass.com به Referer حساسه — handler خودکار Referer و Origin رو set می‌کنه.
        توکن 4 ساعت معتبره — handler در صورت 403 خودش صفحه رو دوباره fetch می‌کنه.
    """
    if not is_rapeinass_url(page_url):
    ...
    if token_remaining is not None and token_remaining < 60:
        ...
        if fresh_html:
            ...
            if new_source:
                ...
                logger.info("[DL-RAPEINASS] Got fresh URL with new token")

    # ── روش 1: multi-segment ──
    logger.info("[DL-RAPEINASS] Attempt 1: multi-segment (%d workers)", DEFAULT_SEGMENT_WORKERS)
    ...
    # اگه 403 یا خطای توکن، صفحه رو دوباره fetch کن
    if error in ("HTTP_403", "Cannot determine file size"):
        logger.info("[DL-RAPEINASS] Token may have expired — refreshing page...")
        if progress_cb:
        ...
        if fresh_html:
            ...
            if new_source:
                video_url = new_source["url"]
                logger.info("[DL-RAPEINASS] Got fresh URL")
                success, error, size = await _download_multi_segment(
                    video_url, filepath, referer, progress_cb, dl_id=dl_id,
                )
                if success:
    ...
    logger.info(f"[DL-RAPEINASS] Multi-segment failed: {error}")
    _cleanup_file(filepath)

    # ── روش 2: single-connection ──
    logger.info("[DL-RAPEINASS] Attempt 2: single-connection")
    success, error, size = await _download_single(
        video_url, filepath, referer, progress_cb, dl_id=dl_id,
    )
    if success:
        return True, "", size
    logger.info(f"[DL-RAPEINASS] Single failed: {error}")
    _cleanup_file(filepath)

    # ── روش 3: yt-dlp روی page URL ──
    logger.info("[DL-RAPEINASS] Attempt 3: yt-dlp on page URL")
...
async def download_rapeinass_direct(
    url: str,
    filepath: str,
    progress_cb: Optional[ProgressCallback] = None,
    video_url: str = "",
    ...
    """
    if not video_url:
        qualities, title, info = await extract_rapeinass_qualities(url, progress_cb)
        if not qualities:
    ...
    return await download_rapeinass_video(
...
async def _cli_main():
    import sys
    if len(sys.argv) < 2:
        print("Usage: python rapeinass_handler.py <url> [output.mp4] [quality]")
        sys.exit(1)

    url = sys.argv[1]
    output = sys.argv[2] if len(sys.argv) > 2 else "/home/z/my-project/download/rapeinass_test.mp4"
