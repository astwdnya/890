...
rapedws_handler.py
──────────────────
استخراج و دانلود ویدیو از raped.ws

روش کار (بر اساس تحلیل واقعی سایت — تأیید شده با تست):

  سایت ساده — بدون پلاگین پلیر اختصاصی، بدون Cloudflare Challenge.

  ساختار URL:
    /content/<slug>.html

  HTML الگو:
    <video id="player" poster="/thumbs/...">
      <source src="" type="video/mp4">   <!-- src خالی است -->
    </video>

    <script>
    var url='aHR0cHM6Ly9jb250ZW50LW9wMS5yYXBlZC53cy8...';   // base64-encoded URL
    document.onreadystatechange = function(){
        if(document.readyState === 'complete'){
          var video = document.getElementById('player');
          video.src = atob(url);   // decode و set کردن src بعد از load
        }
    }
    </script>

  نکته مهم: URL ویدیو به صورت base64-encoded در متغیر JS به نام `url` قرار داره
  و با atob() در مرورگر decode می‌شه.

  CDN (content-op1.raped.ws):
    URL pattern: https://content-op1.raped.ws/<network>/<subdir>/<hash>.mp4
    مثال: https://content-op1.raped.ws/adultmoney/am/665bca559185cc56031cfbf1f032499d-1.mp4

    - Accept-Ranges: bytes (HTTP 206)
    - Content-Type: video/mp4
    - Server: nginx
    - بدون توکن، بدون expiry، بدون IP-lock
    - بدون نیاز به Referer (حتی با fake Origin هم کار می‌کنه!)
    - بدون Cloudflare محافظت

  نکات:
    - سایت تک‌کیفیتیه
    - کوکی set نمی‌شه (یا حداقل مهم نیست)
    - Pattern برای همه ویدیوها یکسانه

استراتژی دانلود:
  1. GET parent page برای metadata و video URL
  2. استخراج متغیر `url` از <script> و decode base64
...
logger = logging.getLogger("RapedWSHandler")
...
def is_rapedws_url(url: str) -> bool:
    """بررسی اینکه URL مربوط به raped.ws هست."""
    try:
        host = urlparse(url).hostname or ""
        return host in _ALLOWED_HOSTS or host.endswith(".raped.ws")
    except Exception:
        return False
...
def extract_video_slug(url: str) -> Optional[str]:
    ...
    الگو: /content/<slug>.html
    """
    try:
        parsed = urlparse(url)
        path = parsed.path or ""
        m = re.search(r"/content/([^/]+?)\.html", path)
        if m:
            return m.group(1)
        return None
    except Exception:
...
def _decode_base64_url(b64_str: str) -> Optional[str]:
    """
    Decode یک base64 string به URL.
    از base64 استاندارد و URL-safe پشتیبانی می‌کنه.
    """
    if not b64_str:
        return None
    # پاکسازی whitespace
    b64_str = b64_str.strip()
    # URL-safe base64 (با - و _ به جای + و /)
    b64_str_normalized = b64_str.replace("-", "+").replace("_", "/")
    # padding
    padding_needed = len(b64_str_normalized) % 4
    if padding_needed:
        b64_str_normalized += "=" * (4 - padding_needed)
    try:
        decoded = base64.b64decode(b64_str_normalized)
...
def _strip_site_suffix(title: str) -> str:
    """حذف پسوندهای سایت."""
    title = re.sub(r"\s*[-|@]\s*raped(?:\.ws)?\s*$", "", title, flags=re.IGNORECASE)
    return title.strip()
...
def _extract_thumbnail(html: str) -> str:
    ...
    if m:
        ...
        # اگه poster مسیر نسبی بود، absolute کن
        if poster and not poster.startswith("http"):
            poster = "https://raped.ws" + (poster if poster.startswith("/") else "/" + poster)
        return poster
    return ""
...
def _extract_video_source(html: str) -> Optional[dict]:
    ...
    سایت URL ویدیو رو به صورت base64-encoded در متغیر JS به نام `url` قرار می‌ده:
        var url='aHR0cHM6Ly9jb250ZW50LW9wMS5yYXBlZC53cy8...';
        video.src = atob(url);

    Returns:
        dict با کلیدهای url, method یا None
    """
    # روش 1: الگوی var url='<base64>'
    patterns = [
        # var url='base64string'
        r"""var\s+url\s*=\s*['"]([A-Za-z0-9+/=_\-]{20,})['"]""",
        # let url='base64string'
        r"""let\s+url\s*=\s*['"]([A-Za-z0-9+/=_\-]{20,})['"]""",
        # const url='base64string'
        r"""const\s+url\s*=\s*['"]([A-Za-z0-9+/=_\-]{20,})['"]""",
        # atob('base64string')
        r"""atob\s*\(\s*['"]([A-Za-z0-9+/=_\-]{20,})['"]\s*\)""",
    ]
    ...
    for pat in patterns:
        ...
        if m:
            b64 = m.group(1)
            decoded = _decode_base64_url(b64)
            if decoded and (".mp4" in decoded.lower() or "mp4" in decoded.lower()):
                url = _clean_url(decoded)
                # validation: باید یه URL معتبر باشه
                try:
                    parsed = urlparse(url)
                    if parsed.scheme in ("http", "https") and parsed.netloc:
                        logger.info("Found video URL from base64 variable: %s", url[:100])
                        return {
                            "url": url,
                            "method": "base64_var_url",
                        }
                except Exception:
                    pass

    # روش 2: پیدا کردن هر base64 string که بعد از decode می‌شه به یه .mp4 URL
    # این fallback برای زمانی که متغیر url نام متفاوتی داره
    b64_pattern = re.compile(r"""['"]([A-Za-z0-9+/=_\-]{40,})['"]""")
    for m in b64_pattern.finditer(html):
        b64 = m.group(1)
        # سریع چک کن اگه بعد از decode شامل mp4 یا http می‌شه
        try:
            decoded = _decode_base64_url(b64)
            if decoded and "http" in decoded.lower()[:10] and ".mp4" in decoded.lower():
                url = _clean_url(decoded)
                try:
                    parsed = urlparse(url)
                    if parsed.scheme in ("http", "https") and parsed.netloc:
                        logger.info("Found video URL from generic base64: %s", url[:100])
                        return {
                            "url": url,
                            "method": "base64_generic",
                        }
                except Exception:
                    pass
        except Exception:
    ...
    # روش 3: جستجوی مستقیم URL در HTML (اگه در JS مستقیماً به جای base64 نوشته شده)
    # الگوی content-op*.raped.ws
    cdn_pattern = re.compile(
        r'(https?://content-op\d+\.raped\.ws[^\s"\'<>)\]]+?\.mp4[^\s"\'<>)\]]*)',
        re.IGNORECASE,
    )
    ...
    for m in mp4_pattern.finditer(html):
        url = _clean_url(m.group(1))
        # فقط از host های raped.ws
        try:
            host = urlparse(url).hostname or ""
            if host.endswith(_CDN_HOST_SUFFIX) or "raped.ws" in host:
        ...
        try:
            async with _CurlAsyncSession() as session:
                ...
                if resp.status_code == 200 and resp.text:
                    text = resp.text
                    if "<html" in text.lower() or "<video" in text.lower() or "var url" in text:
...
async def extract_rapedws_qualities(
    url: str,
    progress_cb: Optional[ProgressCallback] = None,
) -> Tuple[List[dict], str, dict]:
    """
    استخراج کیفیت‌های ویدیو از raped.ws.

    Args:
        url: URL صفحه ویدیو (مثل https://raped.ws/content/<slug>.html)
...
async def _download_multi_segment(
    ...
    try:
        ...
        logger.info(
            "[DL-RAPEDWS] Work-queue: %d chunks, %d workers, total=%d",
        ...
        try:
            ...
            logger.error("[DL-RAPEDWS] Work-queue error: %s", e, exc_info=True)
        ...
        if worker_failures or failed_chunks:
            logger.warning(
                "[DL-RAPEDWS] %d workers failed, %d chunks failed",
        ...
        logger.info(
            "[DL-RAPEDWS] Multi-segment DONE | size=%s | time=%.1fs | avg=%.1f MB/s",
            _format_size(file_size), elapsed, avg_speed,
        )
        return True, "", file_size

    except Exception as e:
        logger.error("[DL-RAPEDWS] Multi-segment error: %s", e, exc_info=True)
        _cleanup_file(filepath)
        return False, str(e)[:200], 0
...
async def _make_worker_task(worker_func, worker_id: int):
    ...
    try:
        ...
        logger.error("[DL-RAPEDWS] Worker %d crashed: %s", worker_id, e)
        return False
        ...
        while True:
            ...
            for attempt in range(max_retries):
                ...
                try:
                    ...
                    logger.warning(
                        "[DL-RAPEDWS] W%d c%d attempt %d failed: %s",
    ...
    if _check_curl_cffi():
        try:
            ...
            async with _CurlAsyncSession() as session:
                ...
                logger.info(
                    "[DL-RAPEDWS] Single (curl_cffi) DONE | size=%s | time=%.1fs",
                    _format_size(size), elapsed,
                )
                return True, "", size

        except Exception as e:
            logger.warning(f"[DL-RAPEDWS] Single (curl_cffi) error: {e}")
            _cleanup_file(filepath)
...
async def _download_with_ytdlp(url: str, filepath: str, progress_cb: ProgressCallback) -> Tuple[bool, str, int]:
    ...
    try:
        cmd = [
            ...
            "--add-header", f"Referer:https://raped.ws/",
            "-o", filepath,
        ]
        if has_curl_cffi:
        ...
        if actual_path != filepath:
            try:
                ...
                pass
        logger.info("[DL-RAPEDWS] yt-dlp DONE | size=%s", _format_size(size))
        return True, "", size
        ...
        logger.error("[DL-RAPEDWS] yt-dlp error: %s", e, exc_info=True)
        _cleanup_file(filepath)
        return False, str(e)[:200], 0
...
async def download_rapedws_video(
    page_url: str,
    video_url: str,
    filepath: str,
    progress_cb: Optional[ProgressCallback] = None,
    ...
    """
    دانلود ویدیو از raped.ws.

    Args:
        page_url: URL صفحه ویدیو (برای Referer)
        video_url: URL مستقیم MP4 (از extract_rapedws_qualities)
    ...
    نکته: CDN به Referer حساس نیست، ولی handler برای اطمینان set می‌کنه.
    """
    if not is_rapedws_url(page_url):
    ...
    # ── روش 1: multi-segment ──
    logger.info("[DL-RAPEDWS] Attempt 1: multi-segment (%d workers)", DEFAULT_SEGMENT_WORKERS)
    ...
    if error in ("HTTP_403", "Cannot determine file size"):
        logger.info("[DL-RAPEDWS] Refreshing page...")
        if progress_cb:
        ...
        if fresh_html:
            ...
            if new_source:
                ...
                logger.info("[DL-RAPEDWS] Got fresh URL")
                success, error, size = await _download_multi_segment(
                    video_url, filepath, referer, progress_cb, dl_id=dl_id,
                )
                if success:
    ...
    logger.info(f"[DL-RAPEDWS] Multi-segment failed: {error}")
    _cleanup_file(filepath)

    # ── روش 2: single-connection ──
    logger.info("[DL-RAPEDWS] Attempt 2: single-connection")
    success, error, size = await _download_single(
        video_url, filepath, referer, progress_cb, dl_id=dl_id,
    )
    if success:
        return True, "", size
    logger.info(f"[DL-RAPEDWS] Single failed: {error}")
    _cleanup_file(filepath)

    # ── روش 3: yt-dlp روی page URL ──
    logger.info("[DL-RAPEDWS] Attempt 3: yt-dlp on page URL")
...
async def download_rapedws_direct(
    url: str,
    filepath: str,
    progress_cb: Optional[ProgressCallback] = None,
    video_url: str = "",
    ...
    """
    if not video_url:
        qualities, title, info = await extract_rapedws_qualities(url, progress_cb)
        if not qualities:
    ...
    return await download_rapedws_video(
...
async def _cli_main():
    import sys
    if len(sys.argv) < 2:
        print("Usage: python rapedws_handler.py <url> [output.mp4] [quality]")
        sys.exit(1)

    url = sys.argv[1]
    output = sys.argv[2] if len(sys.argv) > 2 else "/tmp/rapedws_test.mp4"
