"""
severeporn_handler.py
─────────────────────
استخراج و دانلود ویدیو از severeporn.com

روش کار (بر اساس تحلیل واقعی صفحه):

  ─── ساختار صفحه ───
  سایت از KT Player استفاده می‌کنه (مثل sleazyneasy، fetishshrine، pornwhite، babestube).
  URL های ویدیو در inline JS به‌صورت flashvars object تعریف می‌شن:

    var flashvars = {
      video_id: '287672',
      video_title: '...',
      video_url: 'https://severeporn.com/get_file/8/HASH/287000/287672/287672.mp4/?v-acctoken=TOKEN',
      video_url_text: '480p',
      video_alt_url: 'https://severeporn.com/get_file/8/HASH/287000/287672/287672_720p.mp4/?v-acctoken=TOKEN',
  ...
  ─── ساختار URL ویدیو ───
  https://severeporn.com/get_file/{N}/{HASH}/{DIR_ID}/{VIDEO_ID}/{FILENAME}.mp4/?v-acctoken={BASE64_TOKEN}
    - N: عدد 1-10 (server variant)
    - HASH: MD5-like hash که هر بار عوض می‌شه
    - DIR_ID: {VIDEO_ID rounded down to 1000}000 (مثلاً 287000 برای 287672)
    - FILENAME: {VIDEO_ID}.mp4 یا {VIDEO_ID}_{QUALITY}.mp4
    - v-acctoken: base64-encoded token با TTL کوتاه

  ─── کیفیت‌ها ───
  severeporn از flashvars استفاده می‌کنه:
    - video_url: کیفیت اصلی (معمولاً 480p یا 360p)
    - video_alt_url: کیفیت HD (720p یا 1080p) — اگه video_alt_url_hd=1 باشه
    - video_alt_url2: کیفیت سوم (اختیاری)
  نکته: severeporn معمولاً ۲ کیفیت ارائه می‌ده (default + HD).

  ─── سرور ───
  - severeporn.com: پشت Cloudflare هست ولی aiohttp کار می‌کنه (cf-cache-status: DYNAMIC)
  - PHPSESSID و kt_acctoken کوکی‌ها برای session persistence لازم هستن
  - URL امضا‌دار با TTL کوتاه (باید هر بار fetch کنی)

  ─── رفتار CDN ───
  - Accept-Ranges: bytes ✓ (multi-segment download کار می‌کنه)
  - بعضی از URL های /get_file/ 403 می‌دن (cache miss یا acctoken نامعتبر)
    باید همه URL های candidate رو تست کنیم.
  - yt-dlp با extractor=generic کار می‌کنه

استراتژی دانلود:
  1. fetch صفحه با aiohttp (سریع — حتی با CF)
  2. fallback به curl_cffi با impersonate=chrome
  3. استخراج flashvars از HTML
  4. استخراج URL های ویدیو (video_url + video_alt_url + video_alt_url2)
...
logger = logging.getLogger("SeverePornHandler")
...
# CDN مجاز (همون هاست — ویدیوها از خود دامنه سرو می‌شن)
...
def is_severeporn_url(url: str) -> bool:
    """بررسی اینکه URL مربوط به severeporn هست."""
    try:
        host = urlparse(url).hostname or ""
        return host in _ALLOWED_HOSTS or host.endswith(".severeporn.com")
    except Exception:
        return False
...
def _is_main_video_url(url: str) -> bool:
    ...
    if "stats.php" in url_lower:
        return False
    # فقط /get_file/ با .mp4 — نه preview.mp4
...
def _extract_title(html: str) -> str:
    ...
    if flashvars.get("video_title"):
        ...
        title = re.sub(
            r"\s*[-|@]\s*(?:severeporn\.com|SeverePorn)\s*$",
    ...
    if m:
        ...
        title = re.sub(r"\s*[-|@]\s*(?:severeporn\.com|SeverePorn)\s*$", "", title, flags=re.IGNORECASE)
        return title
...
def _extract_video_sources(html: str) -> List[dict]:
    ...
    اولویت 2: video_alt_url از flashvars
    اولویت 3: video_alt_url2 از flashvars
    اولویت 4: URL های /get_file/ با v-acctoken (regex fallback)
    ...
    if flashvars.get("video_url"):
        ...
        if _is_main_video_url(url) and url not in seen_urls:
            ...
            is_hd = flashvars.get("video_url_hd") == "1" or flashvars.get("video_url_fhd") == "1"
            url_lower = url.lower()
            if "_1080p" in url_lower:
                ...
                height = 480
            elif quality_text and quality_text.endswith("p"):
                try:
                    height = int(quality_text[:-1])
                except ValueError:
                    height = 480
            else:
                height = 480
    ...
    if flashvars.get("video_alt_url"):
        ...
        if _is_main_video_url(url) and url not in seen_urls:
            ...
            logger.info("Found video_alt_url from flashvars (%s): %s", quality_text, url[:100])

    # کیفیت alt2 (video_alt_url2) — اختیاری
    if flashvars.get("video_alt_url2"):
        ...
        if _is_main_video_url(url) and url not in seen_urls:
            ...
            is_hd = flashvars.get("video_alt_url2_hd") == "1"
            url_lower = url.lower()
            if "_1080p" in url_lower:
                quality_text = quality_text or "1080p"
                height = 1080
                ...
                height = 720
                quality_text = quality_text or "alt2"
            ...
            sources.append({
                ...
                "height": height,
                "quality_key": quality_text.lower(),
                "method": "flashvars_alt2",
                "is_hd": is_hd,
            })
            logger.info("Found video_alt_url2 from flashvars (%s): %s", quality_text, url[:100])

    # ─── روش 2: URL های /get_file/ با v-acctoken (fallback) ───
    ...
    for m in vacctoken_pattern.finditer(html):
        ...
        if "_1080p" in url_lower:
            ...
            # اگه fallback پیدا کردیم ولی از flashvars نبود، skip کنیم تا تکراری نشه
            continue
        sources.append({
...
async def extract_severeporn_qualities(url, progress_cb=None):
    """استخراج کیفیت‌های ویدیو."""
    if not is_severeporn_url(url):
        return [], "Invalid URL", {}
...
async def _download_multi_segment(direct_url, filepath, referer, cookies, progress_cb, dl_id="", num_workers=MULTI_SEGMENT_WORKERS):
    ...
    نکته مهم: فایل handle رو خارج از حلقه باز نگه می‌داریم (P0 fix از sleazyneasy).
    """
    try:
        ...
        logger.info(f"[DL-SP] Work-queue: {total_chunks} chunks, {num_workers} workers, total={content_length}")
        ...
        # session اشتراکی + file handle باز نگه داریم (P0 fix)
            ...
            while True:
                ...
                for attempt in range(MAX_RETRIES):
                    ...
                    try:
                        ...
                        logger.warning(f"[DL-SP] W{worker_id} c{c_idx} attempt {attempt+1} failed: {e}")
        ...
        try:
            ...
            # تشخیص failure های واقعی (P0 fix از sleazyneasy)
            real_failures = []
            ...
            if real_failures or failed_chunks:
                logger.warning(
                    f"[DL-SP] {len(real_failures)} worker failures, {len(failed_chunks)} chunks failed"
                )
                _cleanup_file(filepath)
        ...
        logger.info(
            f"[DL-SP] Multi-segment DONE | size={_format_size(file_size)} | time={elapsed:.1f}s | avg_speed={avg_speed:.1f} MB/s"
        )
...
async def _download_with_ytdlp(url, filepath, progress_cb, quality_key=""):
    ...
    try:
        cmd = [
            ...
            "--add-header", "Referer:https://severeporn.com/",
            "-o", filepath,
        ]
        if has_curl_cffi:
...
async def download_severeporn_video(page_url, video_url, filepath, progress_cb=None, cookies=None, dl_id="", quality_key=""):
    """دانلود ویدیو از severeporn."""
    if not is_severeporn_url(page_url):
    ...
    if error == "Cancelled by user":
        return False, error, 0
    if error == "HTTP_403":
        logger.info("[DL-SP] 403, refreshing session...")
        if progress_cb:
        ...
        if jar:
            ...
            # P0 fix: حفظ کیفیت انتخاب شده هنگام refresh
            qualities, _, info = await extract_severeporn_qualities(page_url, progress_cb=None)
            if qualities:
    ...
    # ─ـ روش 3: yt-dlp ──
    logger.info("[DL-SP] Attempt 3: yt-dlp on page URL")
...
async def download_severeporn_direct(url, filepath, progress_cb=None, video_url="", quality="high", dl_id=""):
    """Wrapper برای سازگاری با bot architecture."""
    if not video_url:
        qualities, title, info = await extract_severeporn_qualities(url, progress_cb)
        if not qualities:
        ...
        quality_key = quality
    return await download_severeporn_video(url, video_url, filepath, progress_cb, cookies=cookies, dl_id=dl_id, quality_key=quality_key)
...
async def _self_test():
    """تست خودي هندلر."""
    test_url = "https://severeporn.com/video/287672/tiny-titted-teen-daisy-phoenix-seduces-her-sister-s-boyfriend-into-an-afternoon-fuck/?play=true"
    print(f"\n{'═' * 80}")
    ...
    sources, title, info = await extract_severeporn_qualities(test_url, progress_cb=progress)
