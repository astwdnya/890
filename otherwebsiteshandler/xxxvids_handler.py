...
xxxvids_handler.py
──────────────────
استخراج و دانلود ویدیو از xxx-vids.pro

روش کار (بر اساس تحلیل واقعی سایت — تأیید شده با تست):
  - سایت از FluidPlayer (پلیر HTML5 متن‌باز) استفاده می‌کنه
  - ویدیو به‌صورت مستقیم در HTML embed شده: یه <video id="player"> با یه <source>
  - برخلاف xxxbp، هیچ API ای وجود نداره — URL ویدیو از HTML استخراج می‌شه
  - سایت فقط یه کیفیت برای هر ویدیو ارائه می‌ده (no quality switching)

  URL pattern:
      https://cdn.adultstream.cc/<base64-like-signature>/<file_id>.mp4?token=ip=<ip>~st=<start>~exp=<expire>~acl=/*~hmac=<hex>

  نکات مهم توکن:
    - IP-locked: توکن به IP ای که صفحه رو fetch کرده بایند شده
    - 24 ساعت معتبر (exp - st = 86400)
    - برای دانلود، صفحه باید از همون IP ای که دانلود می‌کنه fetch بشه
      (این به‌صورت پیش‌فرض در handler رعایت می‌شه)
    - Referer لازم نیست (تست شده) ولی برای اطمینان set می‌شه

  CDN (cdn.adultstream.cc):
    - Accept-Ranges: bytes
    - HTTP 206 برای Range request
    - Content-Type: video/mp4
    - بدون محدودیت IP برای دانلود (فقط توکن چک می‌شه)

استراتژی دانلود:
  1. GET صفحه برای استخراج URL ویدیو از <source> tag
  2. multi-segment download با N workers (پیش‌فرض 16)
  3. fallback به single-connection
  4. fallback به yt-dlp روی page URL
...
logger = logging.getLogger("XXXVidsHandler")
...
def is_xxxvids_url(url: str) -> bool:
    """بررسی اینکه URL مربوط به xxx-vids.pro هست."""
    try:
        host = urlparse(url).hostname or ""
        return host in _ALLOWED_HOSTS or host.endswith(".xxx-vids.pro")
    except Exception:
        return False
...
def extract_video_slug(url: str) -> Optional[str]:
    ...
    مثال: /fuck-it-illusion-like-my-stepdad-is-fucking-my-stepsister-234893.html
    → slug = "fuck-it-illusion-like-my-stepdad-is-fucking-my-stepsister"
...
def extract_video_id(url: str) -> Optional[int]:
    ...
    الگو: /<slug>-<id>.html
    مثال: /fuck-it-illusion-...-234893.html → 234893
    """
    try:
        parsed = urlparse(url)
        path = parsed.path or ""
...
def _strip_site_suffix(title: str) -> str:
    """حذف پسوندهای سایت مثل ' - xxx-vids.pro' یا 'xxx video'."""
    # الگوهای رایج در عنوان‌ها:
    # "Title - xxx-vids.pro"
    # "Title xxx video"
    # "Title movie -  xxx-vids.pro"
    title = re.sub(r"\s*[-|@]\s*xxx[-\s]?vids(?:\.pro)?\s*$", "", title, flags=re.IGNORECASE)
    title = re.sub(r"\s+xxx\s*video\s*$", "", title, flags=re.IGNORECASE)
...
def _extract_title(html: str) -> str:
    ...
    # روش 2: JSON-LD VideoObject name
    m = re.search(
        r'"@type"\s*:\s*"VideoObject"[^}]*?"name"\s*:\s*"([^"]+)"',
        html, re.DOTALL,
    )
    if m:
        return _strip_site_suffix(m.group(1).strip())
...
def _extract_video_id(html: str) -> Optional[str]:
    """
    استخراج video_id داخلی سایت از HTML.
    از JSON-LD یا از URL های مرتبط استفاده می‌کنیم.
    """
    # روش 1: از URL های share/embed
    m = re.search(r'/video/(\d+)/', html)
    if m:
        return m.group(1)
    # روش 2: از data-id
    m = re.search(r'data-(?:video-)?id=["\'](\d+)["\']', html, re.IGNORECASE)
    if m:
        return m.group(1)
    return None
...
def _extract_video_source(html: str) -> Optional[dict]:
    """
    استخراج URL ویدیو از <source> tag داخل <video id="player">.

    الگوی مورد انتظار:
        <video id="player" ...>
            <source src="https://cdn.adultstream.cc/.../FILE.mp4?token=..." type='video/mp4'>
        </video>

    Returns:
        dict با کلیدهای url, method, source_type یا None
    """
    # روش 1: پیدا کردن <video id="player"> و سپس <source> داخلش
    # این دقیق‌ترین روشه
    ...
    if video_match:
        ...
        # پیدا کردن اولین <source> داخلش
        source_match = re.search(
            r'<source\b([^>]*)>',
            video_block, re.IGNORECASE,
        )
        if source_match:
            attrs_str = source_match.group(1)
            src = _extract_attr(attrs_str, "src")
            source_type = _extract_attr(attrs_str, "type")
            if src:
                url = _clean_url(src)
                if _is_valid_video_url(url):
                    logger.info("Found video URL from <video id=player>/<source>: %s", url[:100])
                    return {
                        "url": url,
                        "method": "video_source_tag",
                        "source_type": source_type or "video/mp4",
                    }

    # روش 2: هر <source type='video/mp4'> در صفحه
    for sm in re.finditer(
        r'<source\b([^>]*type=[\'"]video/(?:mp4|webm)[\'"][^>]*)>',
        html, re.IGNORECASE,
    ):
        ...
        if src:
            url = _clean_url(src)
            if _is_valid_video_url(url):
                logger.info("Found video URL from <source type=video/mp4>: %s", url[:100])
                return {
                    "url": url,
                    "method": "source_tag_any",
                    "source_type": _extract_attr(attrs_str, "type") or "video/mp4",
                }

    # روش 3: جستجوی مستقیم URL های CDN
    # الگوی: https://cdn.adultstream.cc/<signature>/<file_id>.mp4?token=...
    cdn_pattern = re.compile(
        r'(https?://cdn\d*\.adultstream\.cc/[^\s"\'<>)\]]+?\.mp4\?[^\s"\'<>)\]]*?token=[^\s"\'<>)\]]+)',
        re.IGNORECASE,
    )
    m = cdn_pattern.search(html)
    if m:
        url = _clean_url(m.group(1))
        logger.info("Found video URL from CDN regex: %s", url[:100])
        return {
            "url": url,
            "method": "cdn_regex",
    ...
    # روش 4: هر .mp4 URL با token
    mp4_token_pattern = re.compile(
        r'(https?://[^\s"\'<>)\]]+?\.mp4\?[^\s"\'<>)\]]*?token=[^\s"\'<>)\]]+)',
        re.IGNORECASE,
    )
    m = mp4_token_pattern.search(html)
    if m:
        url = _clean_url(m.group(1))
        if _is_valid_video_url(url):
            logger.info("Found video URL from generic MP4+token regex: %s", url[:100])
            return {
                "url": url,
                "method": "mp4_token_regex",
                "source_type": "video/mp4",
            }

    # روش 5: هر .mp4 URL (آخرین fallback)
    ...
    for m in mp4_pattern.finditer(html):
        ...
        try:
            host = urlparse(url).hostname or ""
            if host in _CDN_HOSTS or host.endswith(".adultstream.cc"):
...
def _is_valid_video_url(url: str) -> bool:
    """بررسی اینکه URL یه ویدیوی اصلی هست (نه preview/trailer)."""
    if not url:
        return False
    url_lower = url.lower()
    # فیلتر preview/preview/thumb/screenshot
    if "preview" in url_lower:
        # اما _preview.mp4 از cdn.adultstream.cc هم استفاده می‌شه برای trailerهای مرتبط
        # ما اینها رو فیلتر می‌کنیم چون معمولاً preview هستن
        if "cdn.adultstream.cc" in url_lower:
            # اگه query string داره (token)، احتمالاً main video هست
            if "?" in url_lower and "token=" in url_lower:
                return True
            return False
        return False
    ...
    if "/thumbs/" in url_lower:
        return False
    if "_preview.mp4" in url_lower and "token=" not in url_lower:
        return False
    # باید .mp4 داشته باشه
...
def _detect_quality_from_url(url: str) -> Tuple[str, int, bool]:
    """
    تشخیص کیفیت از URL یا مسیر.

    سایت xxx-vids.pro معمولاً فقط یک کیفیت داره، ولی اگه کیفیت در URL مشخص باشه
    (مثل _720p)، استخراج می‌کنیم.
    """
    url_lower = url.lower()
    for h in (2160, 1440, 1080, 720, 480, 360, 240, 144):
        if f"_{h}p" in url_lower or f"{h}p.mp4" in url_lower:
            return f"{h}p", h, h >= 720
    # default — سایتی که فقط یک کیفیت داره
...
async def _fetch_page(url: str) -> Tuple[Optional[str], dict, str]:
    """
    GET صفحه ویدیو.

    Returns:
        (html, cookies, error) — در صورت موفقیت error خالی است.
        cookies برای حفظ session (هرچند معمولاً برای دانلود لازم نیست).
    """
    cookies_dict: dict = {}
        ...
        try:
            async with _CurlAsyncSession() as session:
                ...
                if resp.status_code == 200 and resp.text:
                    text = resp.text
                    # صحت‌سنجی: باید <video> یا fluidPlayer یا cdn.adultstream داشته باشه
                    if ("<video" in text or "fluidPlayer" in text
                            or "adultstream" in text.lower()):
...
async def extract_xxxvids_qualities(
    url: str,
    progress_cb: Optional[ProgressCallback] = None,
) -> Tuple[List[dict], str, dict]:
    """
    استخراج کیفیت‌های ویدیو از xxx-vids.pro.

    Args:
        url: URL صفحه ویدیو (مثل https://xxx-vids.pro/<slug>-<id>.html)
        progress_cb: callback async برای گزارش پیشرفت

    Returns:
        (sources, title, info)
        sources: list of dicts با کلیدهای label/url/height/quality_key/method/is_hd
                 (معمولاً فقط 1 آیتم — سایت تک‌کیفیتیه)
        title: عنوان ویدیو
        info: dict با کلیدهای thumbnail/page_url/duration/video_id/cookies
    """
    if not is_xxxvids_url(url):
    ...
    # مرحله 1: GET صفحه
    html, cookies, page_error = await _fetch_page(url)
    if not html:
        return [], f"خطا در دریافت صفحه: {page_error}", {}

    # مرحله 2: استخراج metadata و video source
    ...
    # ساخت sources list (فقط 1 آیتم)
    quality_text, height, is_hd = _detect_quality_from_url(source_info["url"])
    if quality_text == "default":
        # سایتی که فقط یک کیفیت داره — label ساده
...
async def _download_multi_segment(
    ...
    try:
        ...
        logger.info(
            "[DL-XXXVIDS] Work-queue: %d chunks, %d workers, total=%d",
        ...
        try:
            ...
            logger.error("[DL-XXXVIDS] Work-queue error: %s", e, exc_info=True)
        ...
        if worker_failures or failed_chunks:
            logger.warning(
                "[DL-XXXVIDS] %d workers failed, %d chunks failed",
        ...
        logger.info(
            "[DL-XXXVIDS] Multi-segment DONE | size=%s | time=%.1fs | avg=%.1f MB/s",
            _format_size(file_size), elapsed, avg_speed,
        )
        return True, "", file_size

    except Exception as e:
        logger.error("[DL-XXXVIDS] Multi-segment error: %s", e, exc_info=True)
        _cleanup_file(filepath)
        return False, str(e)[:200], 0
...
async def _make_worker_task(worker_func, worker_id: int):
    ...
    try:
        ...
        logger.error("[DL-XXXVIDS] Worker %d crashed: %s", worker_id, e)
        return False
        ...
        while True:
            ...
            for attempt in range(max_retries):
                ...
                try:
                    ...
                    logger.warning(
                        "[DL-XXXVIDS] W%d c%d attempt %d failed: %s",
    ...
    if _check_curl_cffi():
        try:
            ...
            async with _CurlAsyncSession() as session:
                ...
                logger.info(
                    "[DL-XXXVIDS] Single (curl_cffi) DONE | size=%s | time=%.1fs",
                    _format_size(size), elapsed,
                )
                return True, "", size

        except Exception as e:
            logger.warning(f"[DL-XXXVIDS] Single (curl_cffi) error: {e}")
            _cleanup_file(filepath)
...
async def _download_with_ytdlp(url: str, filepath: str, progress_cb: ProgressCallback,
    ...
    try:
        cmd = [
            ...
            "--add-header", f"Referer:https://xxx-vids.pro/",
            "-o", filepath,
        ]
        if has_curl_cffi:
        ...
        if actual_path != filepath:
            try:
                ...
                pass
        logger.info("[DL-XXXVIDS] yt-dlp DONE | size=%s", _format_size(size))
        return True, "", size
        ...
        logger.error("[DL-XXXVIDS] yt-dlp error: %s", e, exc_info=True)
        _cleanup_file(filepath)
        return False, str(e)[:200], 0
...
async def download_xxxvids_video(
    page_url: str,
    video_url: str,
    filepath: str,
    progress_cb: Optional[ProgressCallback] = None,
    ...
    """
    دانلود ویدیو از xxx-vids.pro.

    Args:
        page_url: URL صفحه ویدیو (برای Referer)
        video_url: URL مستقیم MP4 (از extract_xxxvids_qualities)
        filepath: مسیر فایل خروجی
        progress_cb: callback async برای گزارش پیشرفت
        cookies: کوکی‌ها (از extract_xxxvids_qualities — معمولاً برای دانلود لازم نیست)
    ...
    نکته مهم:
        توکن URL به IP کاربر بایند شده. اگه صفحه از IP دیگه‌ای fetch شده باشه،
        دانلود fail می‌شه. برای جلوگیری، حتماً از extract_xxxvids_qualities
        و download_xxxvids_video رو پشت سر هم و از همون IP اجرا کنید.
    """
    if not is_xxxvids_url(page_url):
    ...
    # ── روش 1: multi-segment ──
    logger.info("[DL-XXXVIDS] Attempt 1: multi-segment (%d workers)", DEFAULT_SEGMENT_WORKERS)
    ...
    # اگه 403 یا خطای توکن، صفحه رو دوباره fetch کن تا توکن تازه بگیریم
    if error in ("HTTP_403", "Cannot determine file size"):
        logger.info("[DL-XXXVIDS] Token may have expired — refreshing page...")
        if progress_cb:
            await progress_cb("🔄 **Refreshing session...**")
        fresh_html, fresh_cookies, ferr = await _fetch_page(page_url)
        if fresh_html:
            new_source = _extract_video_source(fresh_html)
            if new_source:
                video_url = new_source["url"]
                cookies.update(fresh_cookies)
                logger.info("[DL-XXXVIDS] Got fresh URL")
                success, error, size = await _download_multi_segment(
                    video_url, filepath, referer, cookies, progress_cb, dl_id=dl_id,
                )
                if success:
    ...
    logger.info(f"[DL-XXXVIDS] Multi-segment failed: {error}")
    _cleanup_file(filepath)

    # ── روش 2: single-connection ──
    logger.info("[DL-XXXVIDS] Attempt 2: single-connection")
    success, error, size = await _download_single(
        video_url, filepath, referer, cookies, progress_cb, dl_id=dl_id,
    )
    if success:
        return True, "", size
    logger.info(f"[DL-XXXVIDS] Single failed: {error}")
    _cleanup_file(filepath)

    # ── روش 3: yt-dlp روی page URL ──
    logger.info("[DL-XXXVIDS] Attempt 3: yt-dlp on page URL")
...
async def download_xxxvids_direct(
    url: str,
    filepath: str,
    progress_cb: Optional[ProgressCallback] = None,
    video_url: str = "",
    ...
    Args:
        ...
        quality: 'high' | 'low' | '<quality_key>' — چون سایت تک‌کیفیتیه،
                 این پارامتر معمولاً تاثیری نداره
        dl_id: شناسه برای cancellation
    """
    if not video_url:
        qualities, title, info = await extract_xxxvids_qualities(url, progress_cb)
        if not qualities:
...
async def _cli_main():
    import sys
    if len(sys.argv) < 2:
        print("Usage: python xxxvids_handler.py <url> [output.mp4] [quality]")
        print("  quality: high (default) | low")
        print("  Note: xxx-vids.pro provides single quality per video.")
        sys.exit(1)

    url = sys.argv[1]
    output = sys.argv[2] if len(sys.argv) > 2 else "/home/z/my-project/download/xxxvids_test.mp4"
