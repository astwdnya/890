"""
pornwhite_handler.py
────────────────────
استخراج و دانلود ویدیو از pornwhite.com

روش کار (بر اساس تحلیل واقعی صفحه با analyze_pornwhite.py):
  - سایت پشت Cloudflare CDN هست ولی challenge فعال نیست (aiohttp مستقیم کار می‌کنه)
  - Player: KT Player (مثل cartoonporn، fetishshrine، babestube، hihentaiporn)
...
کیفیت‌ها (مهم):
  pornwhite از flashvars استفاده می‌کنه ولی الگوی متفاوتی داره:
    - video_url: 'https://...'         → کیفیت اصلی (معمولا 720p HD)
    - video_url_hd: '1'                → flag که می‌گه ویدیو HD هست
  نکته: pornwhite فقط یه کیفیت ارائه می‌ده (default).
  اگه video_url_hd=1 باشه، یعنی default خودش HD هست.

  الگوهای پشتیبانی شده (برای آینده، اگه سایت اضافه کرد):
    - video_alt_url + video_alt_url_text (مثل babestube)
    - video_url2, video_url3 (الگوهای جایگزین)

استراتژی دانلود:
  1. fetch صفحه با aiohttp (سریع — سایت CF challenge نداره)
...
logger = logging.getLogger("PornWhiteHandler")
...
def is_pornwhite_url(url: str) -> bool:
    """بررسی اینکه URL مربوط به pornwhite هست."""
    try:
        host = urlparse(url).hostname or ""
        return host in _ALLOWED_HOSTS or host.endswith(".pornwhite.com")
    except Exception:
        return False
...
def _extract_title(html: str) -> str:
    ...
    if flashvars.get("video_title"):
        ...
        # حذف " - pornwhite.com" یا " - PornWhite"
        title = re.sub(
            r"\s*[-|]\s*(?:pornwhite\.com|PornWhite)\s*$",
...
def _extract_flashvars(html: str) -> dict:
    ...
    # استخراج video_title (که ممکنه ' داشته باشه)
...
def _extract_video_sources(html: str) -> List[dict]:
    ...
    اولویت 1: video_url از flashvars (KT Player)
              — pornwhite فقط video_url داره (default)
              — video_url_hd flag می‌گه آیا HD هست یا نه
    ...
    if flashvars.get("video_url"):
        ...
        if _is_main_video_url(url) and url not in seen_urls:
            seen_urls.add(url)
            # تشخیص کیفیت از URL یا از flashvars flag
            ...
            if "_1080p" in url_lower:
                ...
                # default — اگه video_url_hd=1 باشه، یعنی HD (720p)
                if is_hd:
                    quality_text = "720p"
                    height = 720
                else:
            ...
            sources.append({
                ...
                "url": url,
                "height": height,
                "quality_key": quality_text.lower().replace(" (hd)", "").replace(" ", ""),
                "method": "flashvars",
                "is_hd": is_hd,
            })
...
async def extract_pornwhite_qualities(
    url: str,
...
async def _download_multi_segment(
    ...
    از aiohttp استفاده می‌کنه چون سایت CF challenge نداره.
    """
    try:
        ...
        logger.info(f"[DL-PORNWHITE] Work-queue: {total_chunks} chunks, {num_workers} workers, total={content_length}")
            ...
            while True:
                ...
                for attempt in range(max_retries):
                    ...
                    try:
                        ...
                        logger.warning(f"[DL-PORNWHITE] W{worker_id} c{c_idx} attempt {attempt+1} failed: {e}")
        ...
        try:
            ...
            if worker_failures or failed_chunks:
                logger.warning(f"[DL-PORNWHITE] {len(worker_failures)} workers failed, {len(failed_chunks)} chunks failed")
                _cleanup_file(filepath)
                return False, f"Multi-segment failed: {len(failed_chunks)} chunks", 0

        except Exception as e:
            logger.error(f"[DL-PORNWHITE] Work-queue error: {e}", exc_info=True)
            try:
                await shared_session.close()
            except Exception:
                pass
        ...
        if file_size != content_length:
            logger.warning(f"[DL-PORNWHITE] Size mismatch: expected={content_length}, got={file_size}")

        elapsed = time.time() - start_time
        avg_speed = file_size / elapsed / 1024 / 1024 if elapsed > 0 else 0
        logger.info(f"[DL-PORNWHITE] Multi-segment DONE | size={_format_size(file_size)} | time={elapsed:.1f}s | avg_speed={avg_speed:.1f} MB/s")
        return True, "", file_size

    except Exception as e:
        logger.error(f"[DL-PORNWHITE] Multi-segment error: {e}", exc_info=True)
        _cleanup_file(filepath)
        return False, str(e)[:200], 0
...
async def _download_with_ytdlp(
    ...
    try:
        cmd = [
            ...
            "--add-header", f"Referer:https://pornwhite.com/",
            "-o", filepath,
        ]
        ...
        logger.info(f"[DL-PORNWHITE] yt-dlp DONE | size={_format_size(size)}")
        return True, "", size
        ...
        logger.error(f"[DL-PORNWHITE] yt-dlp error: {e}", exc_info=True)
        _cleanup_file(filepath)
        return False, str(e)[:200], 0
...
async def download_pornwhite_video(
    page_url: str,
    video_url: str,
    filepath: str,
    progress_cb: Optional[ProgressCallback] = None,
    ...
    """
    if not is_pornwhite_url(page_url):
    ...
    # ── روش 1: multi-segment ──
    logger.info("[DL-PORNWHITE] Attempt 1: multi-segment")
    ...
    if error == "HTTP_403":
        ...
        logger.info("[DL-PORNWHITE] 403, refreshing session...")
        if progress_cb:
        ...
        if jar:
            ...
            # re-extract URL
            qualities, _, info = await extract_pornwhite_qualities(
                page_url, progress_cb=None
            )
            if qualities:
                video_url = qualities[0]["url"]
                logger.info("[DL-PORNWHITE] Got fresh URL with v-acctoken")
        # retry multi-segment
        success, error, size = await _download_multi_segment(
            video_url, filepath, referer, cookies, progress_cb, dl_id=dl_id,
        )
        if success:
            return True, "", size
    logger.info(f"[DL-PORNWHITE] Multi-segment failed: {error}")
    _cleanup_file(filepath)

    # ── روش 2: single-connection ──
    logger.info("[DL-PORNWHITE] Attempt 2: single-connection")
    success, error, size = await _download_single_aiohttp(
        video_url, filepath, referer, cookies, progress_cb, dl_id=dl_id,
    )
    if success:
        return True, "", size
    logger.info(f"[DL-PORNWHITE] Single failed: {error}")
    _cleanup_file(filepath)

    # ── روش 3: yt-dlp ──
    logger.info("[DL-PORNWHITE] Attempt 3: yt-dlp on page URL")
...
async def download_pornwhite_direct(
    url: str,
    filepath: str,
    progress_cb: Optional[ProgressCallback] = None,
    video_url: str = "",
    ...
    Args:
        ...
        quality: 'high', 'low', '720p', '480p', 'default'
                 (pornwhite معمولا فقط یه کیفیت داره)
    ...
    return await download_pornwhite_video(
