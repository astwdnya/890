"""
leaksextape_handler.py
──────────────────────
Extract and download videos from leak-sex-tape.com (KT Player)

Strategy:
  - Uses pure aiohttp (No Playwright -> Extremely lightweight)
  - Maintains a persistent CookieJar to bypass v-acctoken validation
  - Extracts main URL specifically from flashvars to avoid preview files
  - Cleans HTML entities from URLs
...
logger = logging.getLogger("LeakSexTapeHandler")
...
def is_leaksextape_url(url: str) -> bool:
    """Check if the URL belongs to leak-sex-tape.com."""
    try:
...
def _clean_url(url: str) -> str:
    """Remove HTML entities and trailing garbage from URLs."""
...
def _extract_title(html: str) -> str:
    ...
    if m:
        ...
        title = re.sub(
            r"\s*[|]\s*Leak[- ]?Sex[- ]?Tape.*$", "", title, flags=re.I
        ).strip()
        if title:
            return title
...
def _extract_video_url(html: str) -> Optional[str]:
    """
    Extract the main video URL from KT Player flashvars.
    Avoids preview files by strictly looking for 'video_url' key or '/get_file/' path.
    """
    # Priority 1: Specific key in flashvars
    fv_match = re.search(
        r"video_url\s*:\s*['\"](https?://[^'\"]+)['\"]", html, re.I
    )
    if fv_match:
        url = _clean_url(fv_match.group(1))
        if "/get_file/" in url and ".mp4" in url:
            return url

    # Priority 2: Fallback generic search
    all_mp4s = re.findall(
    ...
    if all_mp4s:
        for u in all_mp4s:
            ...
            # Strictly ignore preview thumbnails
            if "preview.mp4" not in clean_u:
                return clean_u
...
async def extract_leaksextape_qualities(
    url: str,
    progress_cb: Optional[ProgressCallback] = None,
) -> Tuple[List[dict], str, dict]:
    """
    Extract video qualities.
    Note: Site serves a single MP4 quality.

    Returns:
        (qualities, title, info)
    """
    if not is_leaksextape_url(url):
        return [], "Invalid URL", {}
    ...
    return qualities, title, {"thumbnail": thumbnail}


# ─── Download ───────────────────────────────────────────────


async def download_leaksextape_video(
    page_url: str,
    video_url: str,
    filepath: str,
    progress_cb: Optional[ProgressCallback] = None,
    ...
    Required to bypass v-acctoken validation.

    Returns:
        (success, error_message, file_size)
    """
    if not is_leaksextape_url(page_url):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            async with aiohttp.ClientSession(
                ...
                # Step 1: Fetch the page to capture session cookies
                if progress_cb:
                ...
                async with session.get(
                    ...
                    if 0 < content_length < MIN_VALID_VIDEO_SIZE:
                        ...
                        return False, "Server returned error page (Token invalid)", 0
...
async def download_leaksextape_direct(
    url: str,
    filepath: str,
    progress_cb: Optional[ProgressCallback] = None,
    video_url: str = "",
    ...
    return await download_leaksextape_video(
