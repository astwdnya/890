...
sexxxx_handler.py
─────────────────
استخراج و دانلود ویدیو از sexxxx.video
...
logger = logging.getLogger("SexxxxHandler")
...
def is_sexxxx_url(url: str) -> bool:
    """بررسی اینکه URL مربوط به sexxxx.video هست."""
    try:
        host = urlparse(url).hostname or ""
        return host in _ALLOWED_HOSTS or host.endswith(".sexxxx.video")
    except Exception:
        return False
...
def _strip_site_suffix(title: str) -> str:
    """حذف پسوندهای سایت مثل ' | SexXXX.Video'."""
    title = re.sub(r"\s*[-|@]\s*(?:sexxx(?:\.video)?|sex\s*xxx(?:\.video)?)\s*$",
...
def _detect_quality_from_url(url: str) -> Tuple[str, int, bool]:
    ...
    سایت sexxxx.video معمولاً فقط یک کیفیت داره، ولی اگه کیفیت در URL مشخص باشه
...
async def extract_sexxxx_qualities(
    url: str,
    progress_cb: Optional[ProgressCallback] = None,
) -> Tuple[List[dict], str, dict]:
    """
    استخراج کیفیت‌های ویدیو از sexxxx.video.

    Args:
        url: URL صفحه ویدیو (مثل https://sexxxx.video/video/<slug>/)
...
async def _download_multi_segment(
    ...
    try:
        ...
        logger.info(
            "[DL-SEXXX] Work-queue: %d chunks, %d workers, total=%d",
        ...
        try:
            ...
            logger.error("[DL-SEXXX] Work-queue error: %s", e, exc_info=True)
        ...
        if worker_failures or failed_chunks:
            logger.warning(
                "[DL-SEXXX] %d workers failed, %d chunks failed",
        ...
        logger.info(
            "[DL-SEXXX] Multi-segment DONE | size=%s | time=%.1fs | avg=%.1f MB/s",
            _format_size(file_size), elapsed, avg_speed,
        )
        return True, "", file_size

    except Exception as e:
        logger.error("[DL-SEXXX] Multi-segment error: %s", e, exc_info=True)
        _cleanup_file(filepath)
        return False, str(e)[:200], 0
...
async def _make_worker_task(worker_func, worker_id: int):
    ...
    try:
        ...
        logger.error("[DL-SEXXX] Worker %d crashed: %s", worker_id, e)
        return False
        ...
        while True:
            ...
            for attempt in range(max_retries):
                ...
                try:
                    ...
                    logger.warning(
                        "[DL-SEXXX] W%d c%d attempt %d failed: %s",
    ...
    if _check_curl_cffi():
        try:
            ...
            async with _CurlAsyncSession() as session:
                ...
                logger.info(
                    "[DL-SEXXX] Single (curl_cffi) DONE | size=%s | time=%.1fs",
                    _format_size(size), elapsed,
                )
                return True, "", size

        except Exception as e:
            logger.warning(f"[DL-SEXXX] Single (curl_cffi) error: {e}")
            _cleanup_file(filepath)
...
async def _download_with_ytdlp(url: str, filepath: str, progress_cb: ProgressCallback,
    ...
    try:
        cmd = [
            ...
            "--add-header", f"Referer:https://sexxxx.video/",
            "-o", filepath,
        ]
        if has_curl_cffi:
        ...
        if actual_path != filepath:
            try:
                ...
                pass
        logger.info("[DL-SEXXX] yt-dlp DONE | size=%s", _format_size(size))
        return True, "", size
        ...
        logger.error("[DL-SEXXX] yt-dlp error: %s", e, exc_info=True)
        _cleanup_file(filepath)
        return False, str(e)[:200], 0
...
async def download_sexxxx_video(
    page_url: str,
    video_url: str,
    filepath: str,
    progress_cb: Optional[ProgressCallback] = None,
    ...
    """
    دانلود ویدیو از sexxxx.video.

    Args:
        page_url: URL صفحه ویدیو (برای Referer)
        video_url: URL مستقیم MP4 (از extract_sexxxx_qualities)
        filepath: مسیر فایل خروجی
        progress_cb: callback async برای گزارش پیشرفت
        cookies: کوکی‌ها (از extract_sexxxx_qualities — معمولاً برای دانلود لازم نیست)
    ...
    نکته مهم:
        ...
        دانلود fail می‌شه. برای جلوگیری، حتماً از extract_sexxxx_qualities
        و download_sexxxx_video رو پشت سر هم و از همون IP اجرا کنید.
    """
    if not is_sexxxx_url(page_url):
    ...
    # ── روش 1: multi-segment ──
    logger.info("[DL-SEXXX] Attempt 1: multi-segment (%d workers)", DEFAULT_SEGMENT_WORKERS)
    ...
    if error in ("HTTP_403", "Cannot determine file size"):
        logger.info("[DL-SEXXX] Token may have expired — refreshing page...")
        if progress_cb:
        ...
        if fresh_html:
            ...
            if new_source:
                ...
                logger.info("[DL-SEXXX] Got fresh URL")
                success, error, size = await _download_multi_segment(
                    video_url, filepath, referer, cookies, progress_cb, dl_id=dl_id,
                )
                if success:
    ...
    logger.info(f"[DL-SEXXX] Multi-segment failed: {error}")
    _cleanup_file(filepath)

    # ── روش 2: single-connection ──
    logger.info("[DL-SEXXX] Attempt 2: single-connection")
    success, error, size = await _download_single(
        video_url, filepath, referer, cookies, progress_cb, dl_id=dl_id,
    )
    if success:
        return True, "", size
    logger.info(f"[DL-SEXXX] Single failed: {error}")
    _cleanup_file(filepath)

    # ── روش 3: yt-dlp روی page URL ──
    logger.info("[DL-SEXXX] Attempt 3: yt-dlp on page URL")
...
async def download_sexxxx_direct(
    url: str,
    filepath: str,
    progress_cb: Optional[ProgressCallback] = None,
    video_url: str = "",
    ...
    """
    if not video_url:
        qualities, title, info = await extract_sexxxx_qualities(url, progress_cb)
        if not qualities:
    ...
    return await download_sexxxx_video(
...
async def _cli_main():
    import sys
    if len(sys.argv) < 2:
        print("Usage: python sexxxx_handler.py <url> [output.mp4] [quality]")
        print("  quality: high (default) | low")
        print("  Note: sexxxx.video provides single quality per video.")
        sys.exit(1)

    url = sys.argv[1]
    output = sys.argv[2] if len(sys.argv) > 2 else "/home/z/my-project/download/sexxxx_test.mp4"
