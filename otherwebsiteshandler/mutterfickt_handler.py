"""
mutterfickt_handler.py
───────────────────
استخراج و دانلود ویدیو از fa.mutterfickt.com
...
روش کار (بر اساس تحلیل واقعی سایت — تأیید شده با تست):
  ...
  ساختار دو-مرحله‌ای:
    ...
    2. صفحه‌ی embed: /embed/<id>
       ...
       - الگو:
           <video preload="none" controls poster="https://fa.mutterfickt.com/media/thumbs/<n>/v<id>.webp?<ts>">
  ...
  نکات:
    ...
    - سایت fa.mutterfickt.com نسخه فارسیه؛ نسخه‌های دیگه هم هستن (en, ru, ...)
...
logger = logging.getLogger("MutterficktHandler")
...
def is_mutterfickt_url(url: str) -> bool:
    """بررسی اینکه URL مربوط به mutterfickt.com هست."""
    try:
        host = urlparse(url).hostname or ""
        return host in _ALLOWED_HOSTS or host.endswith(".mutterfickt.com")
    except Exception:
        return False
...
def _strip_site_suffix(title: str) -> str:
    """حذف پسوندهای سایت."""
    # الگوهای رایج: " | Mutterfickt", " - Mutterfickt Top", " - سایت Mutterfickt"
    title = re.sub(r"\s*[-|@]\s*(?:mutterfickt(?:\.com)?|mutterfickt)\s*$",
...
async def extract_mutterfickt_qualities(
    url: str,
    progress_cb: Optional[ProgressCallback] = None,
) -> Tuple[List[dict], str, dict]:
    """
    استخراج کیفیت‌های ویدیو از fa.mutterfickt.com.

    Args:
        url: URL صفحه ویدیو (مثل https://fa.mutterfickt.com/video/<id>/<slug>)
    ...
    if parent_html:
        ...
        title = f"mutterfickt video {video_id}"
        description = ""
...
async def _download_multi_segment(
    ...
    try:
        ...
        logger.info(
            "[DL-MUTTERFICKT] Work-queue: %d chunks, %d workers, total=%d",
        ...
        try:
            ...
            logger.error("[DL-MUTTERFICKT] Work-queue error: %s", e, exc_info=True)
        ...
        if worker_failures or failed_chunks:
            logger.warning(
                "[DL-MUTTERFICKT] %d workers failed, %d chunks failed",
        ...
        logger.info(
            "[DL-MUTTERFICKT] Multi-segment DONE | size=%s | time=%.1fs | avg=%.1f MB/s",
            _format_size(file_size), elapsed, avg_speed,
        )
        return True, "", file_size

    except Exception as e:
        logger.error("[DL-MUTTERFICKT] Multi-segment error: %s", e, exc_info=True)
        _cleanup_file(filepath)
        return False, str(e)[:200], 0
...
async def _make_worker_task(worker_func, worker_id: int):
    ...
    try:
        ...
        logger.error("[DL-MUTTERFICKT] Worker %d crashed: %s", worker_id, e)
        return False
        ...
        while True:
            ...
            for attempt in range(max_retries):
                ...
                try:
                    ...
                    logger.warning(
                        "[DL-MUTTERFICKT] W%d c%d attempt %d failed: %s",
    ...
    if _check_curl_cffi():
        try:
            ...
            async with _CurlAsyncSession() as session:
                ...
                logger.info(
                    "[DL-MUTTERFICKT] Single (curl_cffi) DONE | size=%s | time=%.1fs",
                    _format_size(size), elapsed,
                )
                return True, "", size

        except Exception as e:
            logger.warning(f"[DL-MUTTERFICKT] Single (curl_cffi) error: {e}")
            _cleanup_file(filepath)
...
async def _download_with_ytdlp(url: str, filepath: str, progress_cb: ProgressCallback,
    ...
    try:
        cmd = [
            ...
            "--add-header", f"Referer:https://fa.mutterfickt.com/",
            "-o", filepath,
        ]
        if has_curl_cffi:
        ...
        if actual_path != filepath:
            try:
                ...
                pass
        logger.info("[DL-MUTTERFICKT] yt-dlp DONE | size=%s", _format_size(size))
        return True, "", size
        ...
        logger.error("[DL-MUTTERFICKT] yt-dlp error: %s", e, exc_info=True)
        _cleanup_file(filepath)
        return False, str(e)[:200], 0
...
async def download_mutterfickt_video(
    page_url: str,
    video_url: str,
    filepath: str,
    progress_cb: Optional[ProgressCallback] = None,
    ...
    """
    دانلود ویدیو از fa.mutterfickt.com.

    Args:
        page_url: URL صفحه ویدیو (برای Referer)
        video_url: URL مستقیم MP4 (از extract_mutterfickt_qualities)
    ...
    """
    if not is_mutterfickt_url(page_url):
        ...
        if video_id:
            ...
            if embed_html:
                ...
                if new_source:
                    ...
                    logger.info("[DL-MUTTERFICKT] Got fresh URL with new token")

    # ── روش 1: multi-segment ──
    logger.info("[DL-MUTTERFICKT] Attempt 1: multi-segment (%d workers)", DEFAULT_SEGMENT_WORKERS)
    ...
    if error in ("HTTP_403", "Cannot determine file size"):
        logger.info("[DL-MUTTERFICKT] Token may have expired — refreshing embed...")
        if progress_cb:
        ...
        if video_id:
            ...
            if embed_html:
                ...
                if new_source:
                    video_url = new_source["url"]
                    logger.info("[DL-MUTTERFICKT] Got fresh URL")
                    success, error, size = await _download_multi_segment(
                        video_url, filepath, referer, progress_cb, dl_id=dl_id,
                    )
                    if success:
    ...
    logger.info(f"[DL-MUTTERFICKT] Multi-segment failed: {error}")
    _cleanup_file(filepath)

    # ── روش 2: single-connection ──
    logger.info("[DL-MUTTERFICKT] Attempt 2: single-connection")
    success, error, size = await _download_single(
        video_url, filepath, referer, progress_cb, dl_id=dl_id,
    )
    if success:
        return True, "", size
    logger.info(f"[DL-MUTTERFICKT] Single failed: {error}")
    _cleanup_file(filepath)

    # ── روش 3: yt-dlp روی page URL ──
    logger.info("[DL-MUTTERFICKT] Attempt 3: yt-dlp on page URL")
...
async def download_mutterfickt_direct(
    url: str,
    filepath: str,
    progress_cb: Optional[ProgressCallback] = None,
    video_url: str = "",
    ...
    """
    if not video_url:
        qualities, title, info = await extract_mutterfickt_qualities(url, progress_cb)
        if not qualities:
    ...
    return await download_mutterfickt_video(
...
async def _cli_main():
    import sys
    if len(sys.argv) < 2:
        print("Usage: python mutterfickt_handler.py <url> [output.mp4] [quality]")
        sys.exit(1)

    url = sys.argv[1]
    output = sys.argv[2] if len(sys.argv) > 2 else "/home/z/my-project/download/mutterfickt_test.mp4"
