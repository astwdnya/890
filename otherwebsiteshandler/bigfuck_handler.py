...
bigfuck_handler.py
──────────────────
استخراج و دانلود ویدیو از bigfuck.tv

روش کار (بر اساس تحلیل واقعی صفحه با analyze_bigfuck.py):
  - سایت پشت Cloudflare CDN هست ولی challenge فعال نیست (aiohttp مستقیم کار می‌کنه)
  - Player: AE Player (custom HTML5 player با ae-player.min.js)
  - URL ویدیو از <source> tag داخل <video> میان
  - URL از vcdn02.bigfuck.tv با ساختار:
      https://vcdn02.bigfuck.tv/key=KEY,end=TIMESTAMP/video131/f/ff/HASH.mp4
  - URL دارای expiry timestamp (data-url-exp) — TTL کوتاه، باید هر بار fetch کنی
  - بدون کوکی‌های ضروری برای دانلود
  - سرور از Range request پشتیبانی می‌کنه (HTTP 206, Accept-Ranges: bytes)
  - yt-dlp با extractor=html5 کار می‌کنه
...
استراتژی دانلود:
  ...
  3. استخراج URL از <source> tag (یا fallback به vcdn URLs)
  4. multi-segment download با 16 workers
...
logger = logging.getLogger("BigFuckHandler")
...
def is_bigfuck_url(url: str) -> bool:
    """بررسی اینکه URL مربوط به bigfuck هست."""
    try:
        host = urlparse(url).hostname or ""
        return host in _ALLOWED_HOSTS or host.endswith(".bigfuck.tv")
    except Exception:
        return False
...
def _is_main_video_url(url: str) -> bool:
    ...
    try:
        ...
        if host not in _ALLOWED_HOSTS and host not in _ALLOWED_CDN_HOSTS:
            # icdn05.bigfuck.tv برای preview هست — فیلتر می‌شه
            if host.startswith("icdn"):
                return False
            # اجازه بقیه subdomain های bigfuck.tv
            if not host.endswith(".bigfuck.tv"):
                return False
    except Exception:
        return False
    return True
...
def _extract_title(html: str) -> str:
    ...
    if m:
        ...
        # حذف " — Free Porn Video | BigFuck.TV" یا مشابه
        title = re.sub(
            r"\s*[—–-]\s*Free Porn Video\s*[|]\s*BigFuck\.TV\s*$",
...
def _extract_duration(html: str) -> Optional[int]:
    """استخراج مدت ویدیو از JSON-LD یا meta tag."""
    # روش 1: JSON-LD
    ...
    # روش 2: meta tag
    m = re.search(r'<meta[^>]*(?:video:duration|duration)[^>]*content=["\'](\d+)["\']', html, re.IGNORECASE)
    if m:
        try:
...
def _extract_video_sources(html: str) -> List[dict]:
    ...
    اولویت 1: <source> tag داخل <video> (مهم‌ترین — منبع اصلی bigfuck)
    اولویت 2: vcdn02.bigfuck.tv URLs از HTML
    اولویت 3: سایر MP4 URLs از bigfuck.tv
    ...
    # ─── روش 1: <source> tag داخل <video> (مهم‌ترین) ───
    # پیدا کردن <video> tag با تمام content داخلش
    video_block = re.search(r'<video\b[^>]*>(.*?)</video>', html, re.IGNORECASE | re.DOTALL)
    if video_block:
        video_inner = video_block.group(1)
        ...
        for sm in re.finditer(
            ...
            # تشخیص کیفیت از URL (bigfuck معمولا فقط یه کیفیت داره)
            url_lower = url.lower()
            if "_1080p" in url_lower or "/video1080/" in url_lower:
                label = "📺 MP4 1080p"
                height = 1080
                quality_key = "1080p"
            elif "_720p" in url_lower or "/video720/" in url_lower:
                label = "📺 MP4 720p"
                height = 720
                quality_key = "720p"
            elif "_480p" in url_lower or "/video480/" in url_lower:
                label = "📺 MP4 480p"
                height = 480
                quality_key = "480p"
            elif "_360p" in url_lower or "/video360/" in url_lower:
                label = "📺 MP4 360p"
                height = 360
                quality_key = "360p"
            else:
                # bigfuck از /video131/ استفاده می‌کنه — یه کیفیت واحد
                label = "📺 MP4 (default)"
                height = 720  # فرض می‌کنیم default نزدیک 720p باشه
                quality_key = "default"

            data_url_exp = attrs.get("data-url-exp", "")
            sources.append({
                "label": label,
                "url": url,
                "height": height,
    ...
    # ─── روش 2: پیدا کردن vcdn URLs از HTML ───
    # فرمت: https://vcdn02.bigfuck.tv/key=...,end=TIMESTAMP/video131/f/ff/HASH.mp4
    # URL ممکنه URL-encoded باشه
    vcdn_pattern = re.compile(
        r'(https?://vcdn\d+\.bigfuck\.tv/[^\s"\'<>\)\]]+\.mp4)',
        re.IGNORECASE,
    )
    for m in vcdn_pattern.finditer(html):
        url = _clean_url(m.group(1))
        if not _is_main_video_url(url):
            continue
        if url in seen_urls:
            continue
        # فیلتر download2 URLs (این برای download مستقیم با اسم فایل هست)
        # ولی کار می‌کنه، پس اجازه می‌دیم
        seen_urls.add(url)

        url_lower = url.lower()
        if "/download2:" in url_lower or "download2=" in url_lower:
            label = "📥 MP4 (download link)"
            method = "vcdn_download"
        else:
            label = "📺 MP4 (default)"
            method = "vcdn"
        ...
        logger.info("Found vcdn URL: %s", url[:100])

    # ─── روش 3: سایر MP4 URLs از bigfuck.tv (نه preview) ───
    mp4_pattern = re.compile(
        r'(https?://[a-z0-9-]+\.bigfuck\.tv/[^\s"\'<>\)\]]+?\.mp4)',
        re.IGNORECASE,
    )
    ...
    # مرتب‌سازی: source_tag اول (بهترین)، بعد vcdn، بعد generic
    method_priority = {"source_tag": 0, "vcdn": 1, "vcdn_download": 2, "mp4_generic": 3}
...
async def extract_bigfuck_qualities(
    url: str,
...
async def _download_multi_segment(
    ...
    try:
        ...
        logger.info(f"[DL-BIGFUCK] Work-queue: {total_chunks} chunks, {num_workers} workers, total={content_length}")
            ...
            while True:
                ...
                for attempt in range(max_retries):
                    ...
                    try:
                        ...
                        logger.warning(f"[DL-BIGFUCK] W{worker_id} c{c_idx} attempt {attempt+1} failed: {e}")
        ...
        try:
            ...
            if worker_failures or failed_chunks:
                logger.warning(f"[DL-BIGFUCK] {len(worker_failures)} workers failed, {len(failed_chunks)} chunks failed")
                _cleanup_file(filepath)
                return False, f"Multi-segment failed: {len(failed_chunks)} chunks", 0

        except Exception as e:
            logger.error(f"[DL-BIGFUCK] Work-queue error: {e}", exc_info=True)
            try:
                await shared_session.close()
            except Exception:
                pass
        ...
        if file_size != content_length:
            logger.warning(f"[DL-BIGFUCK] Size mismatch: expected={content_length}, got={file_size}")

        elapsed = time.time() - start_time
        avg_speed = file_size / elapsed / 1024 / 1024 if elapsed > 0 else 0
        logger.info(f"[DL-BIGFUCK] Multi-segment DONE | size={_format_size(file_size)} | time={elapsed:.1f}s | avg_speed={avg_speed:.1f} MB/s")
        return True, "", file_size

    except Exception as e:
        logger.error(f"[DL-BIGFUCK] Multi-segment error: {e}", exc_info=True)
        _cleanup_file(filepath)
        return False, str(e)[:200], 0
...
async def _download_with_ytdlp(
    ...
    try:
        cmd = [
            ...
            "--add-header", f"Referer:https://bigfuck.tv/",
            "-o", filepath,
        ]
        ...
        logger.info(f"[DL-BIGFUCK] yt-dlp DONE | size={_format_size(size)}")
        return True, "", size
        ...
        logger.error(f"[DL-BIGFUCK] yt-dlp error: {e}", exc_info=True)
        _cleanup_file(filepath)
        return False, str(e)[:200], 0
...
async def download_bigfuck_video(
    page_url: str,
    video_url: str,
    filepath: str,
    progress_cb: Optional[ProgressCallback] = None,
    ...
    Args:
        ...
        video_url: URL مستقیم ویدیو (از vcdn02)
        filepath: مسیر ذخیره فایل
        progress_cb: callback برای گزارش پیشرفت
        cookies: کوکی‌های session (اختیاری — bigfuck معمولا نیاز نداره)
        dl_id: download ID برای cancel support

    Returns:
        (success, error_message, file_size)
    """
    if not is_bigfuck_url(page_url):
    ...
    # ── روش 1: multi-segment ──
    logger.info("[DL-BIGFUCK] Attempt 1: multi-segment")
    ...
    if error == "Cancelled by user":
        return False, error, 0
    logger.info(f"[DL-BIGFUCK] Multi-segment failed: {error}")
    _cleanup_file(filepath)

    # ── روش 2: single-connection ──
    logger.info("[DL-BIGFUCK] Attempt 2: single-connection")
    success, error, size = await _download_single_aiohttp(
        video_url, filepath, referer, cookies, progress_cb, dl_id=dl_id,
    )
    if success:
        return True, "", size
    logger.info(f"[DL-BIGFUCK] Single failed: {error}")
    _cleanup_file(filepath)

    # ── روش 3: yt-dlp ──
    logger.info("[DL-BIGFUCK] Attempt 3: yt-dlp on page URL")
...
async def download_bigfuck_direct(
    url: str,
    filepath: str,
    progress_cb: Optional[ProgressCallback] = None,
    video_url: str = "",
    ...
    Args:
        ...
        quality: 'high', 'low', 'default' (bigfuck فقط یه کیفیت داره)
    ...
    if not video_url:
        ...
        # انتخاب کیفیت (bigfuck معمولا فقط یه کیفیت داره)
        selected = None
        for q in qualities:
        ...
        if not selected:
            if quality in ("high", "best", "default"):
                # اولین source (که از source_tag میاد) بهترینه
    ...
    return await download_bigfuck_video(
